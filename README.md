<h1 align="center">Hi 👋, I'm Mohamed Asmaan</h1>
<h3 align="center">Frontend Engineer | React • TypeScript • JavaScript | Building AI-Powered Products</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/mohamed-asmaan-337980234/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://mohamed-asmaan.github.io/">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
  <a href="mailto:asmaan011@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

# 👨‍💻 About Me

- 💼 Frontend Engineer with **2+ years of professional experience**
- 🚀 Building scalable web applications using **React, TypeScript, JavaScript & REST APIs**
- 🤖 Currently building **AI-powered applications**
- 📚 Learning **System Design, Backend Engineering & DSA**
- 🌱 Exploring **Docker, CI/CD, Cloud & Scalable Architectures**
- 💬 Ask me about **React, TypeScript, Frontend Architecture, MERN**

---

# 🛠 Tech Stack

### Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css,tailwind,redux,vite" />
</p>

### Backend

<p>
<img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,mysql,firebase,supabase" />
</p>

### Tools

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,figma,docker,npm" />
</p>

### Currently Learning

<p>
<img src="https://skillicons.dev/icons?i=aws,redis,kubernetes" />
</p>

---

# 🚀 Featured Projects

### 🤖 AI Google Calendar Chrome Extension

> AI-powered Chrome Extension that understands natural language and manages Google Calendar events using LLMs and Google Calendar API.

**Tech**
React • TypeScript • Chrome Extension API • Google Calendar API • OAuth

---

### 🏥 Bangla Health Connect

Healthcare platform supporting multilingual content and CMS-driven architecture.

**Tech**
React • Webflow • JavaScript • REST APIs

---

### 🍕 Pizza Palace

Modern responsive food ordering application.

**Tech**
React • JavaScript • Tailwind CSS

---

### 🔍 SEO DOM Inspector

Developer tool for inspecting SEO elements directly in the browser.

**Tech**
JavaScript • Chrome Extension

---

# 📊 GitHub Stats

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=<YOUR_GITHUB_USERNAME>&show_icons=true&theme=github_dark&hide_border=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=<YOUR_GITHUB_USERNAME>&layout=compact&theme=github_dark&hide_border=true"/>

</p>

---

# 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com?user=<YOUR_GITHUB_USERNAME>&theme=github-dark&hide_border=true"/>

</p>

---

# 📈 Contribution Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=<YOUR_GITHUB_USERNAME>&theme=github-dark&hide_border=true"/>

</p>

---

# 🌐 Connect With Me

- 💼 LinkedIn: <YOUR_LINKEDIN_URL>
- 🌍 Portfolio: <YOUR_PORTFOLIO_URL>
- 💻 GitHub: https://github.com/<YOUR_GITHUB_USERNAME>
- 📧 Email: <YOUR_EMAIL>

---

> *"Building products that solve real problems through clean architecture and great user experiences."*
